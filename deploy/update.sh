...
curl -fsS --max-time 15 "$MASTER_URL/api/vendor/checkin?domain=${DOMAIN}&version=${CURRENT}" >/dev/null 2>&1 || true
...
curl -fsS --max-time 15 "$MASTER_URL/api/vendor/checkin?domain=${DOMAIN}&version=${NEW}" >/dev/null 2>&1 || true
